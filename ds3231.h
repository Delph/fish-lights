...
static int8_t dayOfWeek(const DateTime& dt);
static int16_t dayOfYear(const DateTime& dt);
...
struct DateTime
{
  ...
  bool is_leap_year() const { return (year % 4 == 0 && year % 100 != 0) || year % 400 == 0; }
  ...
  bool is_dst() const
  {
    ...
    if (month == 3 || month == 10)
    {
      const uint8_t days = dom[month-1] + (is_leap_year() && month == 2 ? 1 : 0);
      DateTime last(year, month, days, 0, 0, 0);
      const uint8_t lastDoW = last.wday;
      const uint8_t lastSunday = 31 - lastDoW;

      if (month == 3 && (day > lastSunday || (day == lastSunday && hours >= 1)))
        return true;
      if (month == 10 && (day < lastSunday || (day == lastSunday && hours < 1)))
        return true;
    }
    return false;
  }
...
static uint8_t bcd_encode(const uint8_t v)
{
  return v + 6 * (v / 10);
}

static uint8_t bcd_decode(const uint8_t v)
{
  return v - 6 * (v >> 4);
}
...
public:
  ...
  DateTime getDateTime()
  {
    ...
    dt.month = bcd_decode(twi->read() & 0b11111);
